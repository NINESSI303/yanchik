...
bot.on('ready', () => {
    ...
    setInterval(function () {
        bot.user.setPresence({ game: { name: acitvestatus, status: 'online', type: "STREAMING", url: "https://www.twitch.tv/john_k1994" } });
...
bot.on('message', async message => {
    ...
    dbl.hasVoted(`${message.author.id}`).then(async voteds => {
        if (voteds) {
            if (voted <= Date.now()) {
                ...
                let userzid = userid;
                let guildid = '565582751596675079'
    ...
    */
    ...
    if (marks) {
        ...
        if (!marks.includes('💥') && message.content.toLowerCase() == 'джон я тебя люблю') addMark('💥');
